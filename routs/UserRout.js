    ...
    User.findOne({ email: req.body.email })
      .then((user) => {
        ...
        if (!user) {
          ...
          let isPasswordMatch = bcypt.compareSync(
            req.body.password,
            user.password
          );
          if (!isPasswordMatch) {
...
router.post('/getUser/:userID',verifyToken,
 (req , res) => {
